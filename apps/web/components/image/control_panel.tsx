...
/** Stacks the right pane's controls with the spacing every PDF workspace aside uses. */
...
export interface FieldProps {
  label: string;
  /** Optional id passed through to the label's `htmlFor`, for a single native control child. */
  htmlFor?: string;
...
export interface SliderFieldProps {
  ...
  max: number;
  step?: number;
  /** Appended after the value in the label, e.g. '%'. */
  suffix?: string;
  disabled?: boolean;
  hint?: string;
  id?: string;
...
/** A range input with its current value shown in the label, matching the watermark opacity slider. */
...
/** A row of equal width toggle buttons, e.g. text/image kind or quality/target mode. */
...
/** An error alert or a quiet info banner, matching the caveat text at the top of every panel. */
