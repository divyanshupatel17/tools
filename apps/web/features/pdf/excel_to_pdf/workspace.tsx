...
import { Download, FileText, Loader2, Maximize2, Table, TriangleAlert } from 'lucide-react';
...
export function ExcelToPdfWorkspace() {
  ...
  async function run() {
    ...
    try {
      const processor = await loadProcessor('pdf.excel-to-pdf');
      ...
      setNotice(
        error instanceof Error ? error.message : 'This spreadsheet could not be converted.',
      );
    } finally {
      abort.current = null;
      setProgress(null);
    ...
    <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
      ...
      <aside className="border-border bg-surface flex flex-col gap-4 rounded-2xl border p-4 lg:sticky lg:top-20">
        <p className="bg-cream text-ink/80 rounded-xl p-3 text-xs">
          Lays the first sheet out as a table across as many pages as it needs. Other sheets,
          charts, formatting and formulas (beyond their last calculated value) are not included.
        </p>
