...
import { Download, FileText, Loader2, Maximize2, ScanText, TriangleAlert } from 'lucide-react';
...
export function OcrPdfWorkspace() {
  ...
  async function run() {
    ...
    try {
      const processor = await loadProcessor('pdf.ocr-pdf');
    ...
    <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
      ...
      <aside className="border-border bg-surface flex flex-col gap-4 rounded-2xl border p-4 lg:sticky lg:top-20">
        <ul className="bg-cream text-ink/80 list-disc space-y-1 rounded-xl p-3 pl-6 text-xs">
          <li>Adds a searchable, invisible text layer to a scanned PDF.</li>
        ...
        <Button onClick={run} disabled={!canRun} className="h-12 w-full text-base">
          ...
          {busy ? `Recognising ${percent}%` : 'Recognise text'}
        </Button>
