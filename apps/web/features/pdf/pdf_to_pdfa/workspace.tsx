...
import { Archive, Download, FileText, Loader2, Maximize2, TriangleAlert } from 'lucide-react';
...
export function PdfToPdfaWorkspace() {
  ...
  async function run() {
    ...
    try {
      const processor = await loadProcessor('pdf.pdf-to-pdfa');
    ...
    <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
      ...
      <aside className="border-border bg-surface flex flex-col gap-4 rounded-2xl border p-4 lg:sticky lg:top-20">
        <p className="bg-cream text-ink/80 rounded-xl p-3 text-xs">
          Rasterises every page and rebuilds the PDF with an embedded colour profile and PDF/A2b
          metadata. The result is no longer searchable or selectable text.
        </p>
        ...
        <Button onClick={run} disabled={!canRun} className="h-12 w-full text-base">
          ...
          {busy ? `Converting ${percent}%` : 'Convert to PDF/A'}
        </Button>

        {result && resultFile && (
          <section
            aria-label="PDF/A document"
