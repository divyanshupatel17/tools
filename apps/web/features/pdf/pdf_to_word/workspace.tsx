...
import { Download, FileText, Loader2, Maximize2, TriangleAlert } from 'lucide-react';
...
/** The result is a .docx, not a PDF, so there is nothing here for `PageDetailModal` to render —
 * only the source PDF gets a "View" button. */
export function PdfToWordWorkspace() {
  ...
  async function run() {
    ...
    try {
      const processor = await loadProcessor('pdf.pdf-to-word');
      ...
      setNotice(error instanceof Error ? error.message : 'This PDF could not be converted.');
    } finally {
      abort.current = null;
      setProgress(null);
    }
    ...
    <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
      ...
      <aside className="border-border bg-surface flex flex-col gap-4 rounded-2xl border p-4 lg:sticky lg:top-20">
        <p className="bg-cream text-ink/80 rounded-xl p-3 text-xs">
          Rebuilds headings, paragraphs and lists as an editable .docx from the extracted text.
          Table layout, images and the original page design are not reconstructed.
        </p>
        ...
        <Button onClick={run} disabled={!canRun} className="h-12 w-full text-base">
          ...
          {busy ? `Converting ${percent}%` : 'Convert to Word'}
        </Button>
