...
export function PowerpointToPdfWorkspace() {
  ...
  async function run() {
    ...
    try {
      const processor = await loadProcessor('pdf.powerpoint-to-pdf');
      ...
      setNotice(
        error instanceof Error ? error.message : 'This presentation could not be converted.',
      );
    } finally {
      abort.current = null;
      setProgress(null);
    ...
    <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
      ...
      <aside className="border-border bg-surface flex flex-col gap-4 rounded-2xl border p-4 lg:sticky lg:top-20">
        <p className="bg-cream text-ink/80 rounded-xl p-3 text-xs">
          An outline export: each slide&rsquo;s text, laid out top to bottom on its own page. Shape
          position, images and slide design are not read.
        </p>
