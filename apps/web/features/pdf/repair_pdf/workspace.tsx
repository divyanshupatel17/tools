...
import { Download, FileText, Loader2, Maximize2, TriangleAlert, Wrench } from 'lucide-react';
...
/**
 * Re-parses the document with pdf-lib's relaxed object reader and re-saves a clean structure —
 * see `features/pdf/repair_pdf/processor.ts` for exactly what that can and cannot fix.
 */
export function RepairPdfWorkspace() {
  ...
  async function run() {
    ...
    try {
      const processor = await loadProcessor('pdf.repair-pdf');
      ...
      setNotice(error instanceof Error ? error.message : 'This file could not be repaired.');
    } finally {
      abort.current = null;
      setProgress(null);
    }
    ...
    <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
      ...
      <aside className="border-border bg-surface flex flex-col gap-4 rounded-2xl border p-4 lg:sticky lg:top-20">
        <p className="bg-cream text-ink/80 rounded-xl p-3 text-xs">
          Rebuilds the file&rsquo;s internal structure. Fixes a broken reference table, junk before
          the header, or a missing trailer. It cannot recreate content missing from a file that
          was cut off.
        </p>
        ...
        <Button onClick={run} disabled={!canRun} className="h-12 w-full text-base">
          ...
          {busy ? `Repairing ${percent}%` : 'Repair PDF'}
        </Button>

        {result && resultFile && (
          <section
            aria-label="Repaired document"
          ...
          >
            <span className="min-w-0">
              ...
              <span className="text-muted block text-xs">
                {result.pages} page{result.pages === 1 ? '' : 's'} recovered ·{' '}
