...
/** The source .docx has no preview here — only PDF pages get one — so this workspace only ever
 * offers "View" on the converted result. */
export function WordToPdfWorkspace() {
  ...
  async function run() {
    ...
    try {
      const processor = await loadProcessor('pdf.word-to-pdf');
      ...
      setNotice(error instanceof Error ? error.message : 'This document could not be converted.');
    } finally {
      abort.current = null;
      setProgress(null);
    }
    ...
    <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
      ...
      <aside className="border-border bg-surface flex flex-col gap-4 rounded-2xl border p-4 lg:sticky lg:top-20">
        <p className="bg-cream text-ink/80 rounded-xl p-3 text-xs">
          Lays out the document&rsquo;s paragraphs, headings, lists and tables fresh as a PDF.
          Images, headers, footers, columns and text boxes are not read.
        </p>
